...
def test_available_datasets():
    ...
    assert len(uis.available_datasets(category="education")) < len(
...
def test_format_metadata(mock_metadata):
    ...
    pd.testing.assert_frame_equal(
        uis.format_metadata(mock_metadata), expected, check_names=False
    )
...
def test_format_national_data(mock_national_data):
    ...
    output = uis.format_national_data(
        mock_national_data, indicators_dict, countries_dict
    )
...
def test_format_national_data_with_metadata(mock_national_data):
    ...
    output = uis.format_national_data(
        mock_national_data, indicators_dict, countries_dict, metadata
    )
...
class TestUIS(TestCase):
    ...
    def test_update_info(self):
        ...
        uis_obj._data.update(
            {
                "regions": pd.DataFrame(
                    {"REGION_ID": ["R1", "R2"], "REGION": ["Region 1", "Region 2"]}
                )
            }
        )
        uis_obj._update_info()
    ...
    def test_available_indicators(self):
        ...
        uis_obj = uis.UIS("SDG")
        uis_obj._data = {
            "indicators": {"A": "Indicator A", "B": "Indicator B", "C": "Indicator C"}
        }
        assert uis_obj.available_indicators() == ["A", "B", "C"]
        # test as_names
        assert uis_obj.available_indicators(as_names=True) == [
            "Indicator A",
            "Indicator B",
            "Indicator C",
        ]
    ...
    def test_available_countries(self):
        ...
        uis_obj._data = {
            "countries": {"US": "United States", "CN": "China"},
            "regions": None,
        }
        assert uis_obj.available_countries() == ["US", "CN"]
        assert uis_obj.available_countries(as_names=True) == [
            "United States",
            "China",
        ]  # test as_names
        ...
        with pytest.raises(
            ValueError, match="No regional data available for this dataset"
        ):
            uis_obj.available_countries(region="R1")

        # test with region specified and regional data
        uis_obj._data = {
            "countries": {"US": "United States", "MX": "Mexico", "CN": "China"},
            "regions": pd.DataFrame(
                {
                    "REGION_ID": ["R1", "R1", "R2"],
                    "COUNTRY_ID": ["US", "MX", "CN"],
        ...
        assert uis_obj.available_countries(region="R1", as_names=True) == [
            "United States",
            "Mexico",
        ]
